...
void FugaDns::doConnect() {
    ...
    connect(this, SIGNAL(sig_received(std::string,std::string)),
            this, SLOT(slot_doResolve(std::string,std::string)));
...
void FugaDns::slot_doResolve (string in_type, string in_data) {

    // is message we expect?
    if (in_type != "a_name2tcp") return;

    // extract data from in_data
    vector<string> splitted = split(in_data, ",");
    if (splitted.size() != 3) {
        // invalid message!
        cout << "FugaDns: Invalid message received! (" << splitted.size() << ")" << endl;
        return;
    }

    // emit signal
    string name = splitted[0];
    QHostAddress* ip = new QHostAddress(splitted[1].c_str());
    quint16 port = string2quint16(splitted[2]);
...
void FugaDns::login(std::string in_name, std::string in_password) {
    connect(this, SIGNAL(sig_received(std::string,std::string)),
            this, SLOT(slot_checklogin(std::string,std::string)));
...
void FugaDns::slot_checklogin(string in_type, string in_data) {
    cout << "FugaDns: Got answer to login: " << in_type << endl;

    // successful login?
    if (in_type == "a_login_ok") {
        emit sig_loggedin(0);
        disconnect(this, SIGNAL(sig_received(std::string,std::string)),
                   this, SLOT(slot_checklogin(std::string,std::string)));
    }
